...
class ConversationService:
    ...
    def _analyze_intent(
        ...
        if any(word in message_lower for word in ["deploy", "launch", "publish", "go live"]):
            return {"type": "deploy", "confidence": "high"}

        # Check for testing intent
        if any(word in message_lower for word in ["test", "run tests", "check if it works"]):
            return {"type": "test", "confidence": "high"}

        # Check for refinement intent (modification)
        if any(word in message_lower for word in ["add", "change", "update", "modify", "fix", "remove", "delete"]):
            return {"type": "refine", "confidence": "high"}

        # Check for questions
        if any(word in message_lower for word in ["what", "how", "why", "when", "where", "?", "explain", "show me"]):
        ...
        if project.status in [ProjectStatus.IN_PROGRESS.value, ProjectStatus.DEPLOYED.value]:
